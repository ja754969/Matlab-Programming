...
clear;clc;clf;
figure(1)
%假設鹽度 S=0 時,溫度範圍 1 < t < 30°C,壓力範圍 0 < p < 1000
%計算海水密度 ρstp 隨溫度(T)及壓力的變化,並繪製成三維(T,P, ρstp)圖
subplot(2,1,1)
s1=0;
...
surf(t22,p22,dense2)
title('(B)密度ρstp 隨溫度(T)及壓力的變化');
xlabel('溫度(T)');ylabel('壓力(P)');zlabel('密度(ρstp)');
saveas(gcf,'f1_ML_00781035_EX06.jpg','jpg')

figure(2)
%假設鹽度 S=35,?度為 T=10°C,壓力範圍 0 < p < 1000
%計算密度 ρstp 隨壓力的變化,並繪製成二維(P, ρstp)圖
subplot(3,1,1)
s3=35;
t3=10;
...
plot(t4,dense4)
title('(D)密度ρstp 隨溫度(T)的變化');
xlabel('溫度(T)');ylabel('密度(ρstp)');
%(E) 假設溫度為 T=10°C,壓力為 p=0 bar,鹽度範圍 0 < S < 40
...
plot(s5,dense5)
title('(E)密度 ρstp 隨鹽度(S)的變化');
xlabel('鹽度(psu)');ylabel('密度(ρstp)');
saveas(gcf,'f2_ML_00781035_EX06.jpg','jpg')