...
j1=diag(i1)
j2 = [j1(1,1) 0 0 0;0 j1(2,1) 0 0;0 0 j1(3,1) 0;0 0 0 j1(4,1)]
%試以 magic()函數建立一個 5x5 的方陣,並將它設定給矩陣 AK,
%再以 Matlab 語法驗證矩陣 AK的每一直行(column)、每一橫列(row)、及對角線的總和均為 65。
AK = magic(5)
k1 = sum(AK)
K2 = [sum(AK(1,:)) sum(AK(2,:)) sum(AK(3,:)) sum(AK(4,:))]

AK_ROT=rot90(AK)
K3 = [sum(diag(AK)) sum(diag(AK_ROT))]